-- Gestionale OW - API RPC applicative
-- Il frontend usa queste funzioni invece di orchestrare direttamente le tabelle.
...
revoke all on function public.list_events() from anon;
grant execute on function public.list_events() to authenticated;

-- -----------------------------------------------------------------------------
-- Carica un evento ricostruendo lo stesso modello JSON usato dal frontend attuale
-- -----------------------------------------------------------------------------
create or replace function public.get_event(p_code text)
returns jsonb
language plpgsql
stable
security invoker
...
revoke all on function public.get_event(text) from anon;
grant execute on function public.get_event(text) to authenticated;

-- -----------------------------------------------------------------------------
-- Salvataggio atomico dell'intero evento
-- Riceve lo stesso JSON usato dal frontend. Tutte le operazioni sono parte della
-- stessa transazione della chiamata RPC: in caso di errore viene eseguito rollback.
-- -----------------------------------------------------------------------------
...
begin
  ...
  -- Sostituzione completa dei figli. Le FK ON DELETE CASCADE eliminano ruoli,
  -- sottocategorie e assegnazioni quando vengono rimosse le categorie.
  ...
  for v_category in select value from jsonb_array_elements(coalesce(p_event->'roleCategories', '[]'::jsonb)) loop
    insert into public.role_categories (
      ...
      coalesce(nullif(v_category->>'id', ''), gen_random_uuid()::text),
      v_event_id,
    ...
    for v_subcategory in select value from jsonb_array_elements(coalesce(v_category->'subcategories', '[]'::jsonb)) loop
      insert into public.role_subcategories (
        ...
        coalesce(nullif(v_subcategory->>'id', ''), gen_random_uuid()::text),
        coalesce(nullif(v_category->>'id', ''), ''),
    ...
    for v_role in select value from jsonb_array_elements(coalesce(v_category->'roles', '[]'::jsonb)) loop
      insert into public.roles (
        ...
        coalesce(nullif(v_role->>'id', ''), gen_random_uuid()::text),
        coalesce(nullif(v_category->>'id', ''), ''),
      ...
      -- Assegnazioni base (senza sottocategoria)
      v_idx := 0;
      ...
      -- Assegnazioni per sottocategoria
...
-- Elimina evento
-- -----------------------------------------------------------------------------
create or replace function public.delete_event(p_code text)
returns boolean
language plpgsql
volatile
security invoker
...
revoke all on function public.delete_event(text) from anon;
