...
const blank=()=>({version:3,id:"",createdAt:null,savedAt:null,event:{name:"Nuova gara di nuoto",date:"",venue:"",notes:"",pathNotes:"",athleteTotal:0,athleteDescription:"",athleteDepartures:[]},timeline:[],officials:[],roleCategories:[],refereeNotes:{briefingAthletes:"",briefingJury:"",checklist:[]}});
...
function renderHeader(){
  ...
  $('githubBadge').textContent=config.token?'Token API attivo':'Token non inserito';
  $('githubBadge').classList.toggle('connected',!!config.token);
  $('eventAccessBadge').textContent=state.id?`Evento ${state.id}`:'Nessun evento';
  $('eventAccessBadge').classList.toggle('connected',!!state.id)
}
...
function openAccess(mode){
  ...
  $('eventAccessMessage').textContent='';
  $('eventAccessDialog').showModal()
}
async function confirmAccess(){
  const c=code($('eventCodeInput').value),token=$('eventTokenInput').value.trim();
  if(!c||!token){$('eventAccessMessage').textContent='Codice evento e token API sono obbligatori.';return false}
  config.token=token;OWStorage.saveGithubConfig(config);renderHeader();
  if(accessMode==='new'){
    state=blank();state.id=c;state.createdAt=new Date().toISOString();currentSha=null;dirty=true;render();status(`Evento ${c} pronto. Premi Salva per crearlo.`);return true
  }
  try{
    $('eventAccessConfirmBtn').disabled=true;
    const path=`events/${c}.json`,r=await gh(`/repos/${encodeURIComponent(config.owner)}/${encodeURIComponent(config.repo)}/contents/${path}?ref=${encodeURIComponent(config.branch)}`),data=JSON.parse(unb64(r.content));
    state={...blank(),...data,event:{...blank().event,...(data.event||{})},refereeNotes:{...blank().refereeNotes,...(data.refereeNotes||{})}};
    delete state.secret;state.id=c;currentSha=r.sha;dirty=false;render();status(`Evento ${c} aperto.`);return true
  }catch(e){$('eventAccessMessage').textContent=e.message;return false}finally{$('eventAccessConfirmBtn').disabled=false}
}
async function save(){if(!state.id){status('Crea o apri prima un evento.',true);return}if(!requireToken())return;const btn=$('saveEventBtn');btn.disabled=true;btn.textContent='Salvataggio…';try{const data=structuredClone(state);delete data.secret;data.version=3;data.savedAt=new Date().toISOString();data.createdAt=data.createdAt||data.savedAt;const body={message:`${currentSha?'Aggiorna':'Crea'} evento ${state.id}`,content:b64(JSON.stringify(data,null,2)),branch:config.branch};if(currentSha)body.sha=currentSha;const r=await gh(`/repos/${encodeURIComponent(config.owner)}/${encodeURIComponent(config.repo)}/contents/events/${code(state.id)}.json`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});currentSha=r?.content?.sha||currentSha;state.savedAt=data.savedAt;state.createdAt=data.createdAt;dirty=false;btn.textContent='⇩ Salva';renderHeader();status(`Evento ${state.id} salvato su GitHub.`)}catch(e){btn.textContent='● Salva';status(e.message,true)}finally{btn.disabled=false}}
